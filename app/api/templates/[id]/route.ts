import { AuditAction, AuditEntityType } from "@prisma/client";
import { jsonError, parseBody, requireApiUser } from "@/lib/api";
import { writeAuditLog } from "@/lib/audit";
import { prisma } from "@/lib/prisma";
import { hasPermission } from "@/lib/rbac";
import { templateUpdateSchema } from "@/lib/validators";

export async function GET(_request: Request, { params }: { params: { id: string } }) {
  const auth = await requireApiUser();
  if (auth.error) return auth.error;
  if (!hasPermission(auth.user.role, "TEMPLATE", "READ")) {
    return jsonError("Forbidden", 403);
  }

  const template = await prisma.template.findUnique({ where: { id: params.id } });
  if (!template) {
    return jsonError("Template not found", 404);
  }

  return Response.json(template);
}

export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  const auth = await requireApiUser();
  if (auth.error) return auth.error;
  if (!hasPermission(auth.user.role, "TEMPLATE", "UPDATE")) {
    return jsonError("Forbidden", 403);
  }

  const parsed = await parseBody(request, templateUpdateSchema);
  if (parsed.error) return parsed.error;

  const before = await prisma.template.findUnique({ where: { id: params.id } });
  if (!before) {
    return jsonError("Template not found", 404);
  }

  const updated = await prisma.template.update({ where: { id: params.id }, data: parsed.data });

  await writeAuditLog({
    actorUserId: auth.user.id,
    entityType: AuditEntityType.TEMPLATE,
    entityId: params.id,
    action: AuditAction.UPDATE,
    before,
    after: updated
  });

  return Response.json(updated);
}

export async function DELETE(_request: Request, { params }: { params: { id: string } }) {
  const auth = await requireApiUser();
  if (auth.error) return auth.error;
  if (!hasPermission(auth.user.role, "TEMPLATE", "DELETE")) {
    return jsonError("Forbidden", 403);
  }

  const before = await prisma.template.findUnique({ where: { id: params.id } });
  if (!before) {
    return jsonError("Template not found", 404);
  }

  await prisma.template.delete({ where: { id: params.id } });

  await writeAuditLog({
    actorUserId: auth.user.id,
    entityType: AuditEntityType.TEMPLATE,
    entityId: params.id,
    action: AuditAction.DELETE,
    before
  });

  return Response.json({ success: true });
}
